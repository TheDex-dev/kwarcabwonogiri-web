...
export default function SiagaPage() {
  ...
  useEffect(() => {
    async function loadEvents() {
      try {
        console.log('Fetching siaga events...');
        const data = await getEventsByType('siaga');
  ...
  const filteredEvents = events.filter(event => {
    ...
    const targetsSiaga = event.targetAudience && 
                        Array.isArray(event.targetAudience) && 
                        event.targetAudience.includes('siaga');
    
    const matchesCategory = categoryFilter === 'all' || 
                         (event.category && event.category === categoryFilter);
    
    if (dateFilter === 'all') return matchesSearch && matchesCategory && targetsSiaga;
    ...
    switch(dateFilter) {
      case 'upcoming':
        return matchesSearch && matchesCategory && targetsSiaga && eventDate >= today;
      case 'past':
        return matchesSearch && matchesCategory && targetsSiaga && eventDate < today;
      default:
        return matchesSearch && matchesCategory && targetsSiaga;
    }
  });
    ...
    return (
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="text-center">
          <span className="block text-3xl font-bold text-green-600">{upcomingCount}</span>
        ...
        <div className="text-center">
          <span className="block text-3xl font-bold text-gray-600">{pastCount}</span>
