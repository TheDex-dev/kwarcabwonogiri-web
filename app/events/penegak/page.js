...
export default function PenegakPage() {
  ...
  useEffect(() => {
    async function loadEvents() {
      const data = await getEventsByType('penegak');
      setEvents(data);
      setLoading(false);
    }
    loadEvents();
  ...
  const filteredEvents = events.filter(event => {
    ...
    // Check if the event targets penegak audience
    const targetsPenegak = event.targetAudience && 
                          Array.isArray(event.targetAudience) && 
                          event.targetAudience.includes('penegak');
    
    const matchesCategory = categoryFilter === 'all' || 
                           (event.category && event.category === categoryFilter);
    
    if (dateFilter === 'all') return matchesSearch && matchesCategory && targetsPenegak;
    ...
    switch(dateFilter) {
      case 'upcoming':
        return matchesSearch && matchesCategory && targetsPenegak && eventDate >= today;
      case 'past':
        return matchesSearch && matchesCategory && targetsPenegak && eventDate < today;
      default:
        return matchesSearch && matchesCategory && targetsPenegak;
    }
  });
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">Kegiatan Penegak</h1>
        <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
          Program kegiatan untuk anggota Pramuka tingkat Penegak di Kwartir Cabang Wonogiri
        </p>
      </div>
      ...
      <CreateContentButton eventType="penegak" />
    </div>
  );
}